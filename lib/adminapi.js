...
adminapi.delete('/admin/jam/:id/staff/:staffid/all', authenticated, function(req, res) {
	...
	if (Number.isNaN(jamid) || jamid < 0 || 
		...
		logger.warn(`Someone tried to delete all staff from a jam without proper parameters: ${JSON.stringify(req.params)}`);
		res.status(400).json({message: `Clearing staff requires a positive id for jam and staff`});
	} else {
		var staffObjectToRemove = {
			jamid: parseInt(req.params.id),
			staffid: parseInt(req.params.staffid)
...
adminapi.delete('/admin/jam/:id/musicians/:musicianid/instruments/:instrumentid', authenticated, function(req, res) {
	var jamid = parseInt(req.params.id);
	var musicianid = parseInt(req.params.musicianid);
	var instrumentid = parseInt(req.params.instrumentid);
	if (Number.isNaN(jamid) || jamid < 0 || 
		Number.isNaN(musicianid) || musicianid < 0 ||
		Number.isNaN(instrumentid) || instrumentid < 0) {
		logger.warn(`Someone tried to delete a musician and role without proper parameters: ${JSON.stringify(req.params)}`);
		res.status(400).json({message: `Deleting musican requires a positive id for jam, musician, and instrument!`});
	} else {
