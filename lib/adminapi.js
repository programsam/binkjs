...
adminapi.put('/admin/jam/:id/private', authenticated, function(req, res) {
	var jamid = parseInt(req.params.id);
	logger.debug(`Received body: ${JSON.stringify(req.body)}`);
	if (! req.body || ! req.body.hasOwnProperty('private')) {
		logger.debug(`Someone tried to set privacy for jam ${jamid} without sending an actual private value...`)
		res.status(400).json({message: "Please send a valid JSON object including a desired private boolean."});
	} else {
		Processing.updateJamPrivacy(jamid, req.body.private, function(err, result) {
			if (err) {
				logger.error(`An error occurred while someone trying to set privacy to jam ${jamid} with visibility ${JSON.stringify(req.body.visible)}: ${err}`);
				res.status(500).json(err);
			} else {
				res.json(result);
			}
