...
adminapi.delete('/admin/entity/:type/:id', authenticated, function(req, res) {
	...
	if (Object.keys(entityTypeMap).indexOf(req.params.type) == -1) {
		...
		res.status(501).json({message: `Deleting ${req.params.type} is not implemented yet`});
		...
		// 		res.status(500).json()
		// 	} else {
		// 		res.status(201).end({message: `${type} #{id} was successfully deleted`});
		// 	}
		// });
	}
})
...
adminapi.post('/admin/entity/:type', authenticated, function(req, res) {
	if (Object.keys(entityTypeMap).indexOf(req.params.type) == -1) {
		...
		logger.warn(`Someone tried to create a ${type} without a name!`);
