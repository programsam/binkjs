...
var upload = multer({
	dest: settings.uploads.destination
});
...
adminapi.post('/admin/jam/:id/files', upload.any(), function(req, res) {
	...
	Processing.uploadFiles(jamid, req.files, function(err, result) {
		if (err) {
			logger.error(`Error uploading files to jam #${jamid}: ${JSON.stringify(err)}`)
			res.status(500).json(err);
		} else {
			res.json(result);
		}
		async.each(req.files, function(file, cb) {
			logger.debug(`File ${file.path} handled, removing it now...`)
			fs.unlink(file.path, function(rmerr) {
				if (rmerr) {
					logger.error(`Error removing file ${file.path}: ${JSON.stringify(rmerr)}`);
					cb(rmerr)
				} else {
					logger.debug(`Successfully removed file ${file.path}.`);
					cb();
				}
			})
		}, function(err) {
			if (err) {
				logger.error(`There was an error deleting some file: ${JSON.stringify(err)}`);
			}
		})
	})
})
