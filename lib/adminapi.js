...
adminapi.put('/admin/jam/:jamid/track/:trackid/up', authenticated, function(req, res) {
	...
	Processing.moveTrackUp(trackid, jamid, function(err, result) {
		if (err) {
			logger.error(`Error moving trackid ${trackid} in jam ${jamid} up: ${JSON.stringify(err)}`);
			res.status(500).json(err);
		} else {
			res.json(result);
		} //else no error updating the jam
...
adminapi.put('/admin/jam/:jamid/track/:trackid/down', authenticated, function(req, res) {
	...
	Processing.moveTrackDown(trackid, jamid, function(err, result) {
		if (err) {
			logger.error(`Error moving trackid ${trackid} in jam ${jamid} down: ${JSON.stringify(err)}`);
			res.status(500).json(err);
		} else {
			res.json(result);
		} //else no error updating the jam
