...
adminapi.get('/admin/awscreds', authenticated, function(req, res) {
	var nowMillis = new Date().getTime();
	var thirtyDaysMillis = 30 * 24 * 60 * 60 * 1000;
	var oneMonthFromNow = new Date(nowMillis + thirtyDaysMillis);
	logger.info(`Sending the AWS credentials to an authenticted client. Expiration: ${oneMonthFromNow.toISOString()}`);
	res.json({
		AccessKeyId: settings.s3.accessKeyId, 
		SecretAccessKey: settings.s3.secretAccessKey,
		Token: req.headers.authorization,
		Expiration: oneMonthFromNow.toISOString()
	});
})
...
//TODO: This needs to be an authenticated-guarded URL!
...
function authenticated(req, res, next) {
	...
	if (req.session.admin) {
		logger.silly("This session is admin.")
		next();
	} else if (req.headers.authorization && req.headers.authorization === settings.session_secret) {
		logger.debug(`Session secret passed; authentication bypassed.`);
		next();
	} else {
		logger.warn("This session is NOT admin.");
