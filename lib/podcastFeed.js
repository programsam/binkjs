				...
				Processing.getJamTracks(thisjam, function(trackserr, tracks) {
					if (trackserr) {
						...
						async.each(thisjam.tracks, function(thistrack, trackcb) {
							...
							if (isSet(thistrack.notes)) {
								item.description += ` Jam Notes: "${thistrack.notes}"`
							}
	
							if (thisjam.band?.bandid && thisjam.band.bandid !== -1) {
							...
							if (isSet(thisjam.band) && thisjam.band?.bandid !== -1) {
								item.content += `Performed by ${thisjam.band.name}. `;
							}
	
							if (isSet(thisjam.location) && thisjam.location?.locid !== -1) {
								item.content += `Located at ${thisjam.location.name}`;
							}
	
							if (isSet(thisjam.notes)) {
...
podcastRoutes.get('/feed.php', function(req, res) {
	logger.warn(`Something is still using old feed.php URL!`);
