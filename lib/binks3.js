...
BINKS3.prototype.testConnection = function (callback) {
  if (settings.minio_settings.using_minio) {
    ...
    // Use AWS S3's HeadBucketCommand and send methods
...
BINKS3.prototype.deleteObject = function (passedPrefix, callback) {
  if (settings.minio_settings.using_minio) {
    // Use MinIO's removeObject method
    s3.removeObject(settings.s3.bucket_name, passedPrefix, function(err) {
      if (err) {
        logger.error(`Error! ${JSON.stringify(err)}`);
        callback(err);
      } else {
    ...
    // Use AWS S3's DeleteObjectCommand and send methods
