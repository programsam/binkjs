...
views.get('/views/jam/:id', function(req, res) {
	...
	if (Number.isNaN(jamid)) {
		...
		Processing.viewJam(jamid, req.session.admin, function(err, thisjam) {
			if (err) {
...
views.get('/views/admin/jam/:id/edit/pics', authenticated, function(req, res) {
	...
	loadJamAndThen(jamid, function(err, thisjam) {
		if (err) {
			...
			res.render('admin/jamEntities/pictures', {jam: thisjam, admin: req.session.admin});
		}
	})
})
	...
	client.query("SELECT * from jams where " + onlyPublic + " date like ('%-" + sqlMonth + "-" + sqlDate + "')",
		...
		else //no error
		{
			...
			if (rows.length > 0) //there is something in the array, return it
			{
				...
				function (err)
				{
					res.render('recentAndHistory', {jams:rows, history:true, admin: req.session.admin})
				}
				) //jams are done
			}
...
views.get('/views/recent', function(req, res) {
	Processing.recentJams(req.session.admin, function(err, recentJams) {
		if (err) {
