...
views.get('/views/entity/:type/:id', function(req, res) {
	...
	if (Object.keys(entityTypeMap).indexOf(req.params.type) == -1) {
		...
		res.status(400).json({message: `${req.params.type} id must be a positive number!!`});
...
views.get('/views/admin/entity/:type/:id', authenticated, function(req, res) {
	var entityId = parseInt(req.params.id);
	if (Object.keys(entityTypeMap).indexOf(req.params.type) == -1) {
		logger.warn(`Somebody tried to look up an unsupported entity type: ${req.params.type}`)
		res.status(400).json({message: `${req.params.type} is not a valid BINK type!`});
	} else if (Number.isNaN(entityId) || entityId <= 0) {
		res.status(400).json({message: `${req.params.id} id must be a positive number!`});
		logger.warn(`Someone tried to lookup a ${req.params.id} by an id that was not a number! They looked up the location ID: ${req.params.id}`);
	} else {
		Processing.getEntity(req.params.type, entityId, function(err, result) {
			if (err) {
				res.status(500).json({message: `Error looking up ${req.params.type} with id ${req.params.id}: ${JSON.stringify(err)}`, error: err});
...
views.get('/views/infowindow/:id', function(req, res) {
	...
	if (Number.isNaN(id) || id < 0) {
		...
		logger.warn(`Someone tried to lookup a location by an id that was not a positive number! They looked up the location ID: ${req.params.id}`);
		return;
	} else {
...
views.get('/views/jam/:id', function(req, res) {
	...
	if (Number.isNaN(jamid) || jamid < 0) {
		res.status(400).json({message: "Jam locations must by looked up by jam id; jam id must be a positive number!"});
		logger.warn(`Someone tried to lookup a jam's location by a jamid that was not a positive number! They looked up the location for jam ID: ${req.params.id}`);
		return;
	} else {
...
views.get('/views/admin/jam/:id/edit/musicians', authenticated, function(req, res) {
	var jamid = parseInt(req.params.id);
	if (Number.isNaN(jamid) || jamid < 0) {
		res.status(400).json({message: "Jam musicians must by looked up by jam id; jam id must be a positive number!"});
		logger.warn(`Someone tried to lookup a jam's musicians by a jamid that was not a positive number! They looked up the musicians for jam ID: ${req.params.id}`);
		return;
	} else {
...
views.get('/views/admin/jam/:id/edit/staff', authenticated, function(req, res) {
	var jamid = parseInt(req.params.id);
	if (Number.isNaN(jamid) || jamid < 0) {
		res.status(400).json({message: "Jam staff must by looked up by jam id; jam id must be a positive number!"});
		logger.warn(`Someone tried to lookup a jam's staff by a jamid that was not a positive number! They looked up the staff for jam ID: ${req.params.id}`);
		return;
	} else {
		// to reuse the processing methods for populating a jam,
		// we create a stubbed out jam that just has its ID and pass that.
...
views.get('/views/admin/jam/edit/:id', function(req, res) {
	var jamid = parseInt(req.params.id);
	if (Number.isNaN(jamid) || jamid < 0) {
		res.status(400).json({message: "Jams must by looked up by id; id must be a positive number!"});
		logger.warn(`Someone tried to lookup a jam by a jamid that was not a positive number! They looked jam ID: ${req.params.id}`);
		return;
	} else {
		Processing.loadFullSingleJam(jamid, req.session.admin, function(err, thisjam) {
			if (err) {
...
views.get('/views/admin/jam/:id/edit/pics', authenticated, function(req, res) {
	var jamid = parseInt(req.params.id);
	if (Number.isNaN(jamid) || jamid < 0) {
		res.status(400).json({message: "Jam pics must by looked up by jam id; jam id must be a positive number!"});
		logger.warn(`Someone tried to lookup a jam's pics by a jamid that was not a positive number! They looked up the pics for jam ID: ${req.params.id}`);
		return;
	} else {
		// to reuse the processing methods for populating a jam,
		// we create a stubbed out jam that just has its ID and pass that.
