...
Processing.prototype.recentAndHistoricJams = function(historic, admin, topcb) {
	...
	var historicWhere = `date LIKE ('%-${sqlMonth}-${sqlDate}')`;
...
Processing.prototype.editEntity = function(type, object, topcb) {
...
Processing.prototype.getGeoCordinates = function(topcb) {
	...
	client.query(`SELECT * FROM locations WHERE lat IS NOT NULL AND lon IS NOT NULL`, function(err, rows) {
		client.end();
		if (err) {
