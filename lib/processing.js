...
Processing.prototype.uploadFiles = function(jamid, files, topcallback) {
	...
	async.each(files, function(thisfile, filecb) {
		...
		if (settings.uploads.mimetypes.hasOwnProperty(thisfile.mimetype) &&
				settings.uploads.mimetypes[thisfile.mimetype].hasOwnProperty('dir')) {
			constructedKey += `${settings.uploads.mimetypes[thisfile.mimetype].dir}/`;
			...
			BINKS3.putObject(constructedKey, thisfile, function(err, result) {
				if (err) {
					logger.debug(`Error uploading file to jam ${jamid} with file ${thisfile.originalname}`);
					filecb(err);
				} else {
					logger.debug(`File ${thisfile.originalname} successfully uploaded; inserting it into the db now...`);
					if (settings.uploads.mimetypes[thisfile.mimetype].hasOwnProperty('table')) {
						var tableName = settings.uploads.mimetypes[thisfile.mimetype].table;

						if (tableName === "tracks") {
							thisfile.s3path = `${settings.uploads.mimetypes[thisfile.mimetype].dir}/${jamid}/${thisfile.originalname}`;
							insertNewTrack(jamid, thisfile, filecb);
						} else if (tableName === "pictures") {
							insertNewPic(jamid, thisfile, filecb);
						} else if (tableName === "video") {
							thisfile.w3Path = `${settings.uploads.mimetypes[thisfile.mimetype].dir}/${jamid}/${thisfile.originalname}`;
							insertNewVid(jamid, thisfile, filecb);
						} else {
							filecb({message: `Your settings.json is incorrect for settings.uploads.mimetypes.${thisfile.mimetype}: ${JSON.stringify(settings.uploads.mimetypes[thisfile.mimetype])}.`})
						} //not a tableName we know about
					} else {
						filecb({message: `Your settings.json is incorrect for settings.uploads.mimetypes.${thisfile.mimetype}: ${JSON.stringify(settings.uploads.mimetypes[thisfile.mimetype])}.`});
					} //could not find 'table' property
				} //no problem putting object
			}) //BINKS3.putObject
		} else {
			filecb({message: `File ${thisfile.originalname} has MIME typ ${thisfile.mimetype}. We do not accept this kind of file. Alternatively, check your settings.json.`});
...
function insertNewVid(jamid, thisfile, filecb) {
	let client = sql();
	client.query("SELECT max(num) as maxnum FROM video WHERE jamid = ?", [jamid], function(sqlerr, results) {
		if (sqlerr) {
			client.end();
			logger.error(`Error finding last video in ${jamid}: ${JSON.stringify(sqlerr)}`);
			filecb(sqlerr);
		} else {
			logger.debug(`Results Object: ${JSON.stringify(results)}`)
			var nextnum = 0;
			if (results[0].hasOwnProperty('maxnum'))
				nextnum = results[0].maxnum + 1;
			logger.debug(`Next video will have number ${nextnum}`);
			client.query("INSERT INTO video (jamid, num, title, path) VALUES (?,?,?,?)",
									[jamid, nextnum, thisfile.originalname, thisfile.s3path],
			function(inserterr, results) {
				if (inserterr) {
					logger.error(`Error inserting new video in ${jamid} with track #${nextnum}: ${JSON.stringify(inserterr)}`);
					filecb(inserterr);
				} else {
					filecb(null, results);
				} //no insertion error
			}) //client.query for insertion
		} //no get max number error
	}) //client.query max num
} //insertNewVid

function insertNewTrack(jamid, thisfile, filecb) {
	let client = sql();
	client.query("SELECT max(num) as maxnum FROM tracks WHERE jamid = ?", [jamid], function(sqlerr, results) {
		if (sqlerr) {
			client.end();
			logger.error(`Error finding last track in ${jamid}: ${JSON.stringify(sqlerr)}`);
			filecb(sqlerr);
		} else {
			logger.debug(`Results Object: ${JSON.stringify(results)}`)
			var nextnum = 0;
			if (results[0].hasOwnProperty('maxnum'))
				nextnum = results[0].maxnum + 1;
			logger.debug(`Next track will have number ${nextnum}`);
			client.query("INSERT INTO tracks (jamid, num, title, path) VALUES (?,?,?,?)",
									[jamid, nextnum, thisfile.originalname, thisfile.s3path],
			function(inserterr, results) {
				if (inserterr) {
					logger.error(`Error inserting new track in ${jamid} with track #${nextnum}: ${JSON.stringify(inserterr)}`);
					filecb(inserterr);
				} else {
					filecb(null, results);
				} //no issue inserting new track
			}) //client.query insert new track
		} //no issue finding latest track
	}) //client.query find latest track
} //insertNewTrack
	...
	client.query("SELECT * FROM `tracks` WHERE jamid = ? AND id = ?",
		...
		function(err, results) {
			if (err) {
				...
				if (results.length !== 1) {
					callback(new Error(`Jam #${obj.jamid} and track #${obj.trackid} has more than one result!`));
				} else {
	...
	client.query("SELECT * FROM `pictures` WHERE jamid = ? AND id = ?",
		...
		function(err, results) {
			if (err) {
				...
				if (results.length !== 1) {
					callback(new Error(`Jam #${obj.jamid} and track #${obj.picid} has <> one result!`));
				} else {
	...
	client.query("SELECT * FROM `video` WHERE jamid = ? AND id = ?",
		...
		function(err, results) {
			if (err) {
				...
				if (results.length !== 1) {
					callback(new Error(`Jam #${obj.jamid} and video #${obj.vidid} has <> one result!`));
				} else {
