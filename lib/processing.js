	...
	client.execute("SELECT * FROM tracks where jamid = ?", [jamid], function(sqlerr, tracks) {
		if (sqlerr) {
			...
			async.each(tracks, function(thistrack, trackcb) {
				...
				client.execute("UPDATE tracks SET title = ? WHERE id = ? AND jamid = ?", [newtitle, thistrack.id, jamid],
...
Processing.prototype.deleteLinkingTableEntity = function(type, id, topcb) {
	...
	var client = sql();
	client.execute(query, params, function(sqlerr, result) {
		client.end();
		if (sqlerr) {
...
Processing.prototype.searchAllEntities = function (queryobject, callback) {
	...
	var client = sql();
	var sqlquery = client.execute(query, params,
		function(err, data) {
			client.end();
			if (err) //error while getting the item
			{
