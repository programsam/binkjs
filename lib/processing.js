...
function sql() {
	return mysql.createPool(settings.mysql);
}
...
Processing.prototype.loadFullSingleJam = function(jamid, admin, topcb) {
	...
	function(err, rows) {
		...
		if (err) {
			...
			logger.warn(`Someone looked up a jam id that doesn't exist: ${jamid}`);
			topcb(new Error(`Jam ID ${jamid} does not exist.`));
		} else {
...
Processing.prototype.moveTrackOrVidUp = function(typeAsTable, jamid, itemid, callback) {
	var client = sql();
...
Processing.prototype.moveTrackOrVidDownOld = function(typeAsTable, jamid, itemid, callback) {
	var client = sql();
	logger.debug(`Request to move ${typeAsTable} ${itemid} in jam ${jamid} down`);
...
Processing.prototype.swapTrackOrVid = function(typeAsTable, upOrDown, jamid, itemid, callback) {
	var that = this;
	if (upOrDown === "up" || upOrDown === "down") {
		var client = sql();
		logger.debug(`Request to move ${typeAsTable} ${itemid} in jam ${jamid} down`);
		var selectedItem, swapItem;
		client.query(`SELECT * FROM ${typeAsTable} WHERE jamid = ? ORDER BY num`, [jamid], function(allJamItemsErr, allJamItems) {
			client.end();
			if (allJamItemsErr) {
				logger.error(`[move.down.all] Error when selecting ${typeAsTable} to move down: ${JSON.stringify(allJamItemsErr)}`);
				...
				logger.debug(`Successfully retrieved ${typeAsTable} for jam: ${jamid}...`);
				async.eachOf(allJamItems, function(thisJamItem, index, jamItemCallback) {
					logger.debug(`thisJamItem.id: ${thisJamItem.id} ; itemid: ${itemid} ; ${thisJamItem.id === itemid}`);
					if (thisJamItem.id == itemid) {
						selectedItem = thisJamItem;
						if (upOrDown === "down") {
							swapItem = allJamItems[index+1];
						} else if (upOrDown === "up") {
							swapItem = allJamItems[index-1];
						}
						logger.debug(`Found: ${JSON.stringify(selectedItem)}, swap: ${JSON.stringify(swapItem)}`);
					} else {
						logger.debug(`This is NOT the item: ${JSON.stringify(thisJamItem)}`)
					}
					jamItemCallback();
				}, function(iterateErr) {
					if (iterateErr) {
						logger.error(`Error while iterating: ${iterateErr}`);
						callback(iterateErr);
					} else if (typeof selectedItem === 'undefined') {
						logger.warn(`Someone tried to swap track ${itemid} ${upOrDown} in jam ${jamid} and nothing was found.`);
						callback(new Error(`Could not find track ${itemid} and/or jam ${jamid}!`));
					} else if (typeof swapItem === 'undefined') {
						logger.info(`Someone tried to move ${selectedItem.id} ${upOrDown} in jam ${jamid}. Nothing to swap with. Ignoring.`);
						callback(null, allJamItems);
					} else {
						logger.debug(`We need to swap item ${selectedItem.id} with item ${swapItem.id}`);
						that.swapImplementation(typeAsTable, jamid, selectedItem, swapItem, function(swaperr) {
							if (swaperr) {
								logger.error(`Error during swap: ${swaperr}`);
								callback(swaperr);
							} else {
								callback(null, allJamItems);
							}
				...
				}) //iterate the items in this jam	
			} //no error getting this jam's items
		}); //query getting all items for this jam
	} else {
		callback(new Error(`${upOrDown} is not a valid direction! Please choose 'up' or 'down'.`));
	}
}

Processing.prototype.swapImplementation = function(typeAsTable, jamid, target, swap, cb) {
	var client = sql();
	logger.debug(`Target: UPDATE ${typeAsTable} SET num = ${swap.num} WHERE id = ${target.id};`);
	client.execute(`UPDATE ${typeAsTable} SET num = ? WHERE id = ?`, [swap.num, target.id], function(err, results) {
		if (err) {
			logger.error(`SQL Error: ${err}`);
			cb(err);
		} else {
			logger.debug(`Swap: UPDATE ${typeAsTable} SET num = ${target.num} WHERE id = ${swap.id};`);
			client.execute(`UPDATE ${typeAsTable} SET num = ? WHERE id = ?`, [target.num, swap.id], function(err, results) {
				client.end();
				if (err) {
					logger.error(`SQL Error: ${err}`);
					cb(err);
...
Processing.prototype.searchJamsTable = function(limit, offset, admin, type, id, order, query, topcb) {
	...
	logger.debug(`[Processing.searchJamsTable]: Deterined whereClause: ${whereClause}`)
	...
	logger.debug(`[Processing.searchJamsTable]: whereClause after type: ${whereClause}`)

	var orderBy = "desc";
	if (order === "asc")
		orderBy = "asc";

	logger.debug(`[Processing.searchJamsTable]: Determined orderBy: ${orderBy}`)

	var search, paramArray;

	if (query) {
		search = `%${query}%`;
		logger.debug(`[Processing.searchJamsTable]: Search active. Query is ${search}`)
		whereClause = `${whereClause} and title like(?) `;
		if (columnName) {
			logger.debug(`[Processing.searchJamsTable]: Search active and column is set. Params will be id, search, offset, limit, id, search.`);
			paramArray = [id, search, offset, limit, id, search]
		} else {
			logger.debug(`[Processing.searchJamsTable]: Search active but not column set. Params will be search, offset, limit, search.`);
			paramArray = [search, offset, limit, search];
		}
	} else {
		if (columnName) {
			logger.debug(`[Processing.searchJamsTable]: Search not active, but column set. Params will be id, offset, limit, id.`);
			paramArray = [id, offset, limit, id];
		} else {
			logger.debug(`[Processing.searchJamsTable]: Search not active and column not set. Params will be offset, limit.`);
			paramArray = [offset, limit];
		}
	}
	...
	logger.debug(`[Processing.searchJamsTable]: query: ${query}`);
	logger.debug(`[Processing.searchJamsTable]: paramArray: ${paramArray}`)
	var client = sql();
