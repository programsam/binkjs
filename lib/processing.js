...
Processing.prototype.updateJamPrivacy = function(id, private, callback) {
	let client = sql();
	logger.debug(`Attempting to set jam ${id} to have privacy ${private}...`)
	client.query("UPDATE jams SET private = ? WHERE id = ?",
		[private, id],
		function(err, result) {
		client.end();
		if (err) {
			logger.error(`Error setting jam ${id} to privacy ${private}: ${JSON.stringify(err)}`);
			callback(err);
		} else {
			logger.debug(`Successfully set jam ${id} to privacy ${private}`)
			callback(null, result);
		}
	})
}
...
Processing.prototype.syncMedia = function(params, callback) {
	logger.debug(`Synchronizing ${params.type} for jam ${params.jamid}`);
	let client = sql();
