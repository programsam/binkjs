...
Processing.prototype.getEntity = function (type, id, onlyPublic, callback) {
	...
	{
		console.warn(`Somebody tried to look up an unusual entity: ${type}`)
	...
	else
	{
		...
		client.query(`SELECT * from ${type} where id = ?`,
			...
			{
				console.log(`Error while retrieving a ${type}: ${err}`)
			...
			else //no error
			{
				if (rows.length === 1) //there is something in the array, return it
				{
					...
					client.end()
					console.log(`More/less than one ${type} matches ID ${id}`);
...
// 							if (err) //error while getting the item
// 							{
// 								console.log(`Error assembling jams for ${type} #${id}`);
// 								callback(new Error(`Error assembling jams for ${type} #${id}`), null);
// 							}
// 							else //no error
// 							{
...
// 				{
// 					client.end()
// 					console.log(`There is no ${type} with id ${id}`);
// 					callback(new Error(`There is no ${type} with id ${id}`), null);
// 				}
// 			} //else
// 		}) //query
// 	} //else
