...
api.get('/api/search', function (req, res) {
	...
	let query = `SELECT * FROM jams WHERE ${onlyPublic} order by date ${orderBy} limit ?,?`;
	console.log(`My query: ${query}`);
	client.query(query,
				[offset, limit], function (err, jams) {
					  if (err)
					  {
						  console.log("ERROR Getting recent jams: " + err)
						  res.status(500).end()
					  ...
					  function (err)
					  {
						  if (err) {
							...
							console.log(`This is what I'm sending back: ${JSON.stringify(jams)}`)
							toRet.rows = jams;
							Processing.getTotalJams(toRet, req.session.admin, function(callback) {
								res.json(toRet);
							});
						  client.end()
					  }
