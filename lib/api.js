...
api.get('/api/jam/:id', function (req, res) {
	...
	client.query('SELECT * from jams where id = ?', [req.params.id], 
		...
		else
		{
			...
			{
				res.statusCode(404).send("This item was not found in BINK!")
			}
		}
	})//outer client.query
}) //get /jam/id
				...
				else if (rows.length == 1)
				{
					...
					if (req.params.type == "musicians")
					{
						...
						client2.query("select distinct jams.title as title, jams.id as id from musiciansoncollection, " +
								...
								"musiciansoncollection.musicianid = musicians.id and jams.private = 0",
							[req.params.id],
					...
					else if (req.params.type == "staff")
					{
						...
						client2.query("select distinct jams.title as title, jams.id as id from productiononcollection, " +
								...
								"productiononcollection.staffid = staff.id and jams.private = 0",
							[req.params.id],
							function(err, jams, fields) {
							if (err) //error while getting the item
							{
					...
					else if (req.params.type == "locations")
					{
						...
						client2.query("select jams.id as id, jams.title as title from jams, " +
								"locations where locations.id = jams.locid and locations.id = ? and jams.private = 0",
							[req.params.id],
							function(err, jams, fields) {
							if (err) //error while getting the item
							{
					...
					else if (req.params.type == "bands")
					{
						var client2 = sql()
						client2.query("select jams.id as id, jams.title as title from jams, " +
								"bands where bands.id = jams.bandid and bands.id = ? and jams.private = 0",
							[req.params.id],
							function(err, jams, fields) {
							if (err) //error while getting the item
							{
...
api.get('/api/timelineData', function(req, res) {
	var client = sql();
	client.query("SELECT id, title as content, date as start from jams where jams.private = 0", 
		function(err, rows, fields) {
		if (err) //error while getting the item
		{
			console.log("ERROR: " + err)
...
api.get('/api/search/:size/:page/:query', function (req, res) {
	res.set('Content-Type','application/json')
	var size = 10
	var page = 0
...
api.get('/api/search/:size/:page', function (req, res) {
	res.set('Content-Type','application/json')
	var size = 10
	var page = 0
	if (req.params.size != "" && req.params.size != null)
