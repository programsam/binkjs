		...
		else
		{
			if (rows.length == 1)
			{
				...
				async.series([
				    ...
				    function(callback)
				    {
				    	Processing.getPrivate(thisjam, callback)
				    },
				    function(callback)
				    {
...
api.get('/api/search/:size/:page/:query', function (req, res) {
	...
	Processing.getTotalSearchJams(query, toRet, req.session.admin)
	var onlyPublic = "private = 0 and ";
	if (req.session.admin)
		onlyPublic = ""
	client.query("SELECT * from jams where " + onlyPublic + "title like (?) order by date desc limit ?,?",
...
api.get('/api/search/:size/:page', function (req, res) {
	...
	Processing.getTotalJams(toRet, req.session.admin)
	var onlyPublic = "where private = 0"
	if (req.session.admin)
		onlyPublic = ""
	client.query("SELECT * from jams " + onlyPublic + " order by date desc limit ?,?", 
				[offset, size], 
				function(err, jams) {
					  if (err)
					  {
